...
Class Connections {
	...
	{
		$sql = $this->db->prepare("select * from admin where username=:name and password=:password");
			/* execute statement */
	 ...
	 function Update($id,$table)
    {
                ...
                $sql = $this->db->prepare("UPDATE $table SET 	status='1' where ID=:id");
                $sql->bindParam(':id', $id);
                $sql->execute();
                if($sql)
                {
...
function Add_Notes($branch,$subject,$sem,$title,$desc,$file)
    {
$sql = $this->db->prepare("INSERT INTO  notes(`branch_id`, `subject_id`, `semister`,`notes_title`,`notes`,`descrption`)
VALUES(:branch,:subject,:sem,:title,:file,:desc)");
